...
export default function AddData() {
  ...
  const handleSubmit = (e) => {
    ...
    // city 
    let cityValue=e.target[6].value;
    if(!city){
      setCityError(true);
      isValid=false;
    }else{
    ...
    if (isValid) {
      if (id) {
        ...
        const newData = { u_id, name, email, date ,city };
        const localData = JSON.parse(localStorage.getItem("userData")) || [];
    ...
    <>
      <div className="form">
        ...
        <form onSubmit={handleSubmit} className="user-form mt-5">
          ...
          <div className=" fs-4">
            <label>Hobby :</label>
            <input type="checkbox" name="hobby-study" value={"study"} />
            Study
          ...
          <div>
            ...
            {cityError ? <p style={{ color: "red" }}>please select city</p> : ""}
          </div>

          {/* age  */}
          <div>
            <label className="fs-4">Age :</label>
            <input type="number" />
          </div>
          ...
          {/* agree terms  */}
          <div className="d-flex ">
            <input type="checkbox" name="agree" value={"agree"} /> I agree with
            term on service.
          </div>
